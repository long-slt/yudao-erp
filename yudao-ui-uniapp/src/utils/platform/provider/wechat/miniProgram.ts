import AuthUtil from '@/service/member/auth'
import SocialApi from '@/service/member/social'
import UserApi from '@/service/member/user'
...
const login = async () => {
  return new Promise(async (resolve, reject) => {
    ...
    // 2. 社交登录
    const loginResult = await AuthUtil.socialLogin(
      socialType,
      codeResult.code,
      'default'
    )
...
const bind = () => {
  return new Promise(async (resolve, reject) => {
    ...
    // 2. 绑定账号
    const bindResult = await SocialApi.socialBind(
      socialType,
      codeResult.code,
      'default'
    )
